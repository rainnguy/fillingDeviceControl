...
(function() {
	lyGrid = (function(params) {
						...
						if (!column[o].hide || column[o].hide == undefined) {
							...
							if (l_tree.tree) {
								...
								if(CommnUtil.in_array(lt,column[o].colkey)){
									 ...
									 var  tlr='';
									 if(tte){
									   tlr=" tree_nolines_minus";
									 }
									 divtree.className = "ly_tree"+tlr;
		...
		var datatree = function() { // 页数
			var evt = arguments[0] || window.event;
			var treediv = evt.srcElement || evt.target;
			var ttr = treediv.parentElement.parentElement.getAttribute('d-tree');
			var tdv = $(treediv).attr("class");
			if(tdv.indexOf("nolines_plus")>-1){
				$(treediv).removeClass("tree_nolines_plus");
				$(treediv).addClass("tree_nolines_minus");
				$("tr[d-tree^='"+ttr+"-']").show();
			}else{ 
				$(treediv).removeClass("tree_nolines_minus");
				$(treediv).addClass("tree_nolines_plus");
				$("tr[d-tree^='"+ttr+"-']").hide(); 
			}
		};
